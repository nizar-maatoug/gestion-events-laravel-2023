...
@section('content')
    ...
    @if((Auth::check())&&(Auth::user()->id === $eventSportif->user_id))
        <a type="button" class="btn btn-primary" href="{{route('eventSportifs.edit',[$eventSportif])}}">Modifier</a>
        <form style="display: inline;" action="{{ route('eventSportifs.destroy', [$eventSportif]) }}" method="post">
            @csrf
            @method('DELETE')
